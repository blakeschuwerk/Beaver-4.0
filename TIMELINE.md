...
## 2026-06-25 — Multi-platform scraper reliability test + embedded-link following

| Field | Detail |
|-------|--------|
| **Phase** | Cross-cutting (F2/F3 reliability, test coverage) |
| **Observed** | The pipeline had only ever been exercised against 2 counties (Sonoma/Legistar, Nash/CivicPlus), one per platform, never under a deliberate test matrix. User asked for proof the scraper "reliably works" across CivicPlus, Legistar, and crawl4ai (the catch-all for everything else), and gave full autonomy to find, fix, and re-test until true. Separately, inspecting real downloaded PDFs showed Legistar agendas are mostly a thin shell linking out to the real attachment documents (`gateway.aspx?M=F&ID=...pdf`) — production had a half-built, unfiltered, untested attempt at following these (`extract_pdf_links()`, first-5-links-no-filter) that would grab Zoom/Teams links before any real document. |
| **Decided** | Add 5 new real counties (2 more CivicPlus, 2 more Legistar, 1 crawl4ai-only/NovusAgenda) to `config/counties.json` and run the full discovery→download→extract pipeline against all 7, fixing whatever broke. For embedded links: build a shared, filtered `extract_embedded_links()` rather than trust the unfiltered production version, wire it into both production and the local test harness, and validate against real data before declaring it done. |
| **Did** | Added Denton TX, Centre PA, Pima AZ, Milwaukee WI, Brazos TX to `config/counties.json`. Fixed 8 real bugs (see DEBUG-LOG.md #3–10): Legistar `timezone` requirement, civic_scraper's `KeyError: 'Minutes'` (worked around via `asset_list=["Agenda"]`), crawl4ai's PDF-link filter missing handler-style URLs (NovusAgenda), a stalled-download-fed-to-Docling failure mode, a 16+ minute Docling hang (now bounded by a 240s timeout in the test harness), a silent mock-fallback in `extract_text()` (now raises `DoclingExtractionError` unless `MOCK_MODE=true` — same anti-pattern as DEBUG-LOG #1, caught before it shipped), and a `sys.modules` collision in the test harness's dual-package loader. Then built `extract_embedded_links()` in `functions/scraper/src/scrapers.py` (reuses/broadens `DOCUMENT_LINK_PATTERN` to also cover `.ppt(x)`/`.xls(x)`), wired into production (`functions/scraper/src/main.py`, replacing the old unfiltered version) and the test harness (`scripts/local_pipeline.py`, refactored into a shared `_process_one_document()` helper, one level of recursion). Added 6 new unit tests for the link filter + 4 for `extract_text()`'s fail-loud behavior. |
| **Verified** | Two independent full-matrix runs: 7/7 documents across all 3 platform types, reproducibly. A third run (raised to 3 docs/county) confirmed embedded-link following against real data: Pima County and Milwaukee County both correctly followed 2 real embedded attachment links each (21/21 documents, reproducible across 2 runs). All 23 Python unit tests pass (9 analyzer, 14 scraper), `pnpm build` green. Known gaps, documented not fixed: crawl4ai's link pattern over-matches nav URLs containing "agenda" in the path (harmless, self-heals); Docling cannot parse legacy pre-2007 binary Office formats (DEBUG-LOG #12, low priority, no county tested has this as a meaningful fraction of documents); embedded-link following is one level deep only; the magic-byte/timeout safety nets added to the test harness were *also* ported to production's `main.py` for the embedded-link path and the top-level download path, but production still lacks the Docling-hang timeout (Cloud Run's own request timeout is the backstop there). |
